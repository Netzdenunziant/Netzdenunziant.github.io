...
function tryJsonMethod() {
    var innerHTML = document.querySelector('html, body').innerHTML;
...
// RND & DuMont
function fickRND() {
    var article = document.querySelector('.pdb-article-body-paidcontentintro, .pay.wall');
    if(!article) {
        console.log('Unpaywall', 'No article element');
        return false;
    }
    ...
    if(!text) {
        console.log('Unpaywall', 'No JSON text');
        return false;
    }
    document.querySelector('.pdb-article-body-paidcontentintro .pdb-richtext-field p, p.selectionShareable').textContent = text;
    article.classList = [];
    return true;
}
